...
# add by Yuanwei 2019-1-25 testing HRA
first_dense_speed  = [10*10*64 + Num_cellState, 512]
second_dense_speed = [first_dense_speed[1], Num_action]

first_dense_overtake  = [10*10*64 + Num_cellState, 512]
second_dense_overtake = [first_dense_overtake[1], Num_action]

first_dense_lanechange  = [10*10*64 + Num_cellState, 512]
second_dense_lanechange = [first_dense_lanechange[1], Num_action]
# end by Yuanwei 2019-1-25

# Path of the network model
load_path = '../saved_networks/2019-01-25_11_38_DQN_both/model.ckpt'
#/home/weiy/manage_code/DRL_based_SelfDrivingCarControl/saved_networks/2019-01-25_11_38_DQN_both
...
with tf.variable_scope('network'):
    ...
    # add by Yuanwei 2019-1-18 testing HRA
...
h_fc1_speed = tf.nn.relu(tf.matmul(h_concat, w_fc1_speed)+b_fc1_speed)
output_speed = tf.matmul(h_fc1_speed,  w_fc2_speed)+b_fc2_speed

h_fc1_overtake = tf.nn.relu(tf.matmul(h_concat, w_fc1_overtake)+b_fc1_overtake)
output_overtake = tf.matmul(h_fc1_overtake,  w_fc2_overtake)+b_fc2_overtake

h_fc1_lanechange = tf.nn.relu(tf.matmul(h_concat, w_fc1_lanechange)+b_fc1_lanechange)
...
# add by Yuanwei 2019-1-18 testing HRA
h_fc1_target_speed = tf.nn.relu(tf.matmul(h_concat_target, w_fc1_target_speed)+b_fc1_target_speed)
output_target_speed = tf.matmul(h_fc1_target_speed,  w_fc2_target_speed)+b_fc2_target_speed

h_fc1_target_overtake = tf.nn.relu(tf.matmul(h_concat_target, w_fc1_target_overtake)+b_fc1_target_overtake)
output_target_overtake = tf.matmul(h_fc1_target_overtake,  w_fc2_target_overtake)+b_fc2_target_overtake

h_fc1_target_lanechange = tf.nn.relu(tf.matmul(h_concat_target, w_fc1_target_lanechange)+b_fc1_target_lanechange)
...
def select_action(progress, sess, observation_stack, state_stack, Epsilon):
    if progress == "Observing":
        ...
        if random.random() < Epsilon:
            ...
            Q_value_speed = output_speed.eval(feed_dict={x_image: [observation_stack], x_sensor: [state_stack]})
            Q_value_overtake = output_overtake.eval(feed_dict={x_image: [observation_stack], x_sensor: [state_stack]})
            Q_value_lanechange = output_lanechange.eval(feed_dict={x_image: [observation_stack], x_sensor: [state_stack]})
...
def train(Replay_memory, sess, step):
    ...
    # add by Yuanwei 2018-1-18 testing HRA
    y_batch_speed = []
    Q_target_speed = output_target_speed.eval(feed_dict = {x_image: observation_next_batch, x_sensor: state_next_batch})
    
    y_batch_overtake = []
    Q_target_overtake = output_target_overtake.eval(feed_dict = {x_image: observation_next_batch, x_sensor: state_next_batch})
    
    y_batch_lanechange = []
    Q_target_lanechange = output_target_lanechange.eval(feed_dict = {x_image: observation_next_batch, x_sensor: state_next_batch})
    # end by Yuanwei 2019-1-18
    ...
    for i in range(len(minibatch)):
        if terminal_batch[i] == True:
            ...
            # add by Yuanwei 2019-1-10 testing HRA with Sara update
            #y_batch_speed.append(reward_batch[i] + Gamma * (1.0 / (1.0*len(action_batch))) * np.sum(Q_target_speed[i]))
            #y_batch_overtake.append(reward_batch[i] + Gamma * (1.0 / (1.0*len(action_batch))) * np.sum(Q_target_overtake[i]))
            #y_batch_lanechange.append(reward_batch[i] + Gamma * (1.0 / (1.0*len(action_batch))) * np.sum(Q_target_lanechange[i]))
            # end by Yuanwei 2019-1-10
